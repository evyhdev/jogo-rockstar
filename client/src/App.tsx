...
function useExitConfirmation(active = true) {
  useEffect(() => {
    if (!active) return;
...
function Master() {
  ...
  return <main className="master-screen"><div className="master-content"><header className="master-header"><div><p className="eyebrow">Painel do mestre</p><h1 className="mt-1 text-3xl font-black text-amber-100">Operação Rockstar</h1><p className="text-sm text-slate-300">Protocolo Seguro</p></div><div className="master-header-actions"><div className="room-code"><p>SALA DA OPERAÇÃO</p><b>{game.id}</b></div><ExitGameButton /></div></header><ErrorMessage message={error} />
    ...
    {(game.phase === "playing" || game.phase === "finished") ? <>{game.phase === "finished" ? <Podium game={game} /> : <div className="master-game-layout"><GuildSidebar game={game} side="left" /><section className="master-panel board-panel"><div className="mb-4 flex items-end justify-between gap-4"><div><p className="eyebrow">Mapa do reino</p><h2 className="mt-1 text-2xl font-black text-amber-100">Trilha da operação</h2><p className="mt-1 text-sm text-slate-300">{game.roundStage === "waiting" && !overlayView ? "Clique na casa destacada para abrir a pergunta." : "A casa atual está em andamento."}</p></div><p className="text-sm text-slate-300">Casa {question?.boardPosition} de 10</p></div><div className="board-stage"><Board game={game} moving={moving} onRevealQuestion={() => setRoundOverlay({ round: game.currentRound, view: "house" })} />{overlayView === "house" ? <div className="board-overlay"><div className="house-reveal"><img src={houseImagePath(question?.boardPosition ?? 1)} alt={question?.house} decoding="async" /><p className="eyebrow mt-2">Casa {question?.boardPosition} · {question?.theme}</p><h2 className="mt-2 text-4xl font-black text-amber-100">{question?.house}</h2><button className="master-button mt-6" onClick={() => setRoundOverlay({ round: game.currentRound, view: "question" })}>Continuar</button></div></div> : null}{overlayView === "question" ? <div className="board-overlay question-overlay"><div className="question-overlay-content"><p className="eyebrow">{question?.theme}</p><h2 className="mt-2 text-3xl font-black text-amber-100">{question?.house}</h2><p className="mt-4 text-lg leading-8 text-slate-100">{question?.statement}</p><div className="mt-5 grid gap-2">{question?.options.map((option) => <div className={`question-option ${game.roundStage === "result" ? getAnswerQualityClass(option.quality) : ""}`} key={option.id}><b>{option.id})</b> {option.text}{game.roundStage === "result" ? <><p className="mt-1 text-xs font-bold">Qualidade: {option.quality}%</p><p className="mt-1 text-xs">{option.justification}</p></> : null}</div>)}</div>{game.roundStage === "result" ? <p className="result-callout"><b>Resposta ideal: {question?.correctOptionId}.</b> {question?.explanation}</p> : null}<div className="question-controls"><p className="text-xs uppercase tracking-widest text-cyan-200">Tempo da rodada</p><p className="mt-1 text-5xl font-black text-amber-300">{game.roundStage === "question" ? seconds : ROUND_SECONDS}<span className="text-xl">s</span></p><button className="master-button mt-4 w-full" disabled={game.roundStage !== "waiting"} onClick={() => confirmAction("Iniciar o cronômetro desta rodada?", () => void run(startRound))}>Iniciar cronômetro</button><button className="master-button secondary mt-3 w-full" disabled={game.roundStage !== "question"} onClick={() => confirmAction("Encerrar as respostas e mostrar os resultados?", () => void run(showResult))}>Mostrar resultados</button><button className="master-button accent mt-3 w-full" disabled={game.roundStage !== "result" || moving} onClick={() => confirmAction("Avançar para a próxima casa?", advance)}>{moving ? "Avançando..." : "Próxima casa"}</button></div></div></div> : null}</div></section><GuildSidebar game={game} side="right" /></div>}</> : null}
  </div>
  </main>;
}
...
function PowerActions({ game, team, run }: { game: GameState; team: Team; run: (action: (draft: GameState) => void) => void }) {
  ...
  const invoke = (payload?: Parameters<typeof usePower>[2]) => confirmAction(`Usar ${character.powerName}? Esta ação não pode ser desfeita.`, () => run((draft) => usePower(draft, team.id, payload)));
...
function Player() {
  ...
  if ((game.phase === "finished" || game.roundStage !== "result") && team.eliminated) return <main className="mx-auto max-w-xl space-y-4 px-4 py-6"><header className={panel}><div className="player-header-actions"><div><p className="text-xs text-cyan-300">{team.name}</p><h1 className="mt-2 text-2xl font-black">{team.leaderName}</h1>{character ? <p className="mt-2 text-amber-300">{character.emoji} {character.name}</p> : null}</div><ExitGameButton /></div></header><ErrorMessage message={error} /><PlayerStatus team={team} /><GameOver /></main>;
  return <main className="mx-auto max-w-xl space-y-4 px-4 py-6"><header className={panel}><div className="player-header-actions"><div><p className="text-xs text-cyan-300">{team.name}</p><h1 className="mt-2 text-2xl font-black">{team.leaderName}</h1>{character ? <p className="mt-2 text-amber-300">{character.emoji} {character.name}</p> : null}</div><ExitGameButton /></div></header><ErrorMessage message={error} />
    ...
    {game.phase === "character-selection" ? <section className={panel}><h2 className="text-xl font-bold">Escolha de personagem</h2>{activeSelector === team.id ? <div className="mt-4 grid gap-3">{available.map((entry) => <button className="rounded-xl bg-slate-900 p-4 text-left" key={entry.id} onClick={() => confirmAction(`Escolher ${entry.name}? Esta ação não pode ser desfeita.`, () => void run((draft) => selectCharacter(draft, teamId, entry.id)))}><b>{entry.emoji} {entry.name}</b><p className="mt-1 text-sm">{entry.specialty}</p><p className="mt-1 text-xs text-amber-300">{entry.powerName}: {entry.powerDescription}</p></button>)}</div> : <p className="mt-4">Aguarde {game.teams[activeSelector]?.leaderName}.</p>}</section> : null}
    {(game.phase === "playing" || game.phase === "finished") ? <><PlayerStatus team={team} />{character ? <section className={panel}><div className="rounded-xl bg-slate-900 p-3"><b>{character.powerName}</b><p className="mt-1 text-sm">{character.powerDescription}</p><p className="mt-2 text-xs text-amber-300">{team.powerUsed ? "Poder utilizado" : "Disponível uma vez"}</p></div></section> : null}{game.phase === "finished" ? <section className={panel}><h2 className="text-2xl font-bold text-amber-300">Reino Protegido</h2><p className="mt-3">Sua posição: {getRanking(game).findIndex((entry) => entry.id === team.id) + 1}º lugar.</p></section> : <section className={panel}><div className="flex justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-cyan-300">{question?.theme}</p><p className="mt-1 text-xs text-slate-300">{question?.house}</p></div><p className="text-amber-300">{game.roundStage === "question" ? seconds : ROUND_SECONDS}s</p></div>{game.roundStage === "waiting" ? <p className="mt-4">Aguarde o mestre iniciar o cronômetro.</p> : null}{game.roundStage === "question" ? <><h2 className="mt-3">{question?.statement}</h2><PowerActions game={game} team={team} run={run} /><div className="mt-4 grid gap-2">{question?.options.map((option) => <button key={option.id} disabled={seconds === 0 || Boolean(team.answerId) || team.annulledOptionIds.includes(option.id)} className={`${button} text-left ${selectedAnswer === option.id ? "bg-amber-400 text-slate-950 ring-2 ring-amber-200" : "bg-slate-900"}`} onClick={() => setSelectedAnswer(option.id)}>{option.id}) {team.annulledOptionIds.includes(option.id) ? "Alternativa anulada pelo Tiro Preciso" : option.text}</button>)}</div>{team.answerId ? <div className="mt-4 rounded-xl bg-emerald-500/15 p-3 text-emerald-200"><p className="text-center font-bold">Resposta {team.answerId} confirmada.</p><p className="mt-2 text-sm">{question?.options.find((option) => option.id === team.answerId)?.text}</p></div> : <button className={`${button} mt-4 w-full bg-emerald-500 text-slate-950`} disabled={!selectedAnswer || seconds === 0 || team.annulledOptionIds.includes(selectedAnswer)} onClick={() => selectedAnswer && confirmAction(`Enviar a resposta ${selectedAnswer}? Esta ação não pode ser desfeita.`, () => void run((draft) => submitAnswer(draft, teamId, selectedAnswer)))}>Confirmar resposta{selectedAnswer ? ` ${selectedAnswer}` : ""}</button>}</> : null}{game.roundStage === "result" ? <div className={`answer-result mt-4 ${getAnswerQualityClass(team.results[question!.id]?.quality)}`}><p className="font-bold">Sua resposta: {team.results[question!.id]?.answerId ?? "não respondida"}</p>{team.results[question!.id]?.answerId ? <p className="mt-2 text-sm">{question?.options.find((option) => option.id === team.results[question!.id]?.answerId)?.text}</p> : null}<p className="mt-3">Qualidade: {team.results[question!.id]?.quality ?? 0}% · +{team.results[question!.id]?.scoreGained} ponto(s) · +{team.results[question!.id]?.leakGained}% vazamento</p><p className="mt-3 text-sm">{question?.explanation}</p><PowerActions game={game} team={team} run={run} /></div> : null}</section>}</> : null}
  </main>;
}
